...
def tensors_to_video(tensors, output_video, framerate=24):
    """
    Convert a list of tensors directly to a video file using ffmpeg.
    
    Parameters:
    tensors (list): List of PyTorch tensors representing video frames.
    output_video (str): Path to the output video file.
    framerate (int): Frame rate of the output video.
    """
    # Convert tensors to numpy arrays
    frames = [tensor.cpu().numpy() for tensor in tensors]

    # Stack frames along a new dimension
    video_array = np.stack(frames, axis=0)

    # Get dimensions from the first frame (assuming all frames have the same dimensions)
    height, width, _ = video_array[0].shape

    # Convert from CHW (if necessary) to HWC format
    ...
    process = (
        ffmpeg
        .input('pipe:', format='rawvideo', pix_fmt='rgb24', s='{}x{}'.format(width, height), framerate=framerate)
        .output(output_video, pix_fmt='yuv420p', vcodec='libx264')
        .overwrite_output()
        .run_async(pipe_stdin=True)
    )

    # Write frames to the ffmpeg process
